...
export function AudioPlayer({ element }: AudioPlayerProps) {
  ...
  timeRef.current = player.timeStore.getCurrentTime();

  // Save playback snapshot before unmount so GlobalBGMPlayer can resume seamlessly.
  useEffect(() => {
    return () => {
      ...
      sessionStorage.setItem(PLAYER_HANDOFF_KEY, JSON.stringify(payload));
    };
  }, []);
