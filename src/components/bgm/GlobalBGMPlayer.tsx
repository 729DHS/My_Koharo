...
import { $bgmActiveTab, $bgmPanelOpen, closeBgmPanel, setBgmActiveTab } from '@/store/bgm';

const PLAYER_HANDOFF_KEY = 'audio-player-handoff';
const HANDOFF_MAX_AGE_MS = 60_000;

interface PlayerHandoffSnapshot {
  url: string;
  name?: string;
  artist?: string;
  time: number;
...
export default function GlobalBGMPlayer({ audioGroups }: GlobalBGMPlayerProps) {
  ...
  useEffect(() => {
    const shouldResumeFromHandoff = (() => {
      if (typeof window === 'undefined') return false;
      const raw = sessionStorage.getItem(PLAYER_HANDOFF_KEY);
      if (!raw) return false;
      try {
        const data = JSON.parse(raw) as PlayerHandoffSnapshot;
        return Date.now() - data.timestamp <= HANDOFF_MAX_AGE_MS;
      } catch {
        sessionStorage.removeItem(PLAYER_HANDOFF_KEY);
        return false;
      }
    })();

    if ((!panelOpen && !shouldResumeFromHandoff) || resolvedRetryRef.current === retryKey) return;
  ...
  // Resume playback handoff from inline AudioPlayer when navigating back to regular pages.
  useEffect(() => {
    if (handoffAppliedRef.current || tracks.length === 0 || typeof window === 'undefined') return;

    const raw = sessionStorage.getItem(PLAYER_HANDOFF_KEY);
    if (!raw) return;

    let snapshot: PlayerHandoffSnapshot | null = null;
    try {
      snapshot = JSON.parse(raw) as PlayerHandoffSnapshot;
    } catch {
      sessionStorage.removeItem(PLAYER_HANDOFF_KEY);
      return;
    }

    if (!snapshot || Date.now() - snapshot.timestamp > HANDOFF_MAX_AGE_MS) {
      sessionStorage.removeItem(PLAYER_HANDOFF_KEY);
      return;
    }

    const indexByUrl = tracks.findIndex((track) => track.url === snapshot.url);
    const indexByMeta = tracks.findIndex((track) => track.name === snapshot.name && track.artist === snapshot.artist);
    const targetIndex = indexByUrl >= 0 ? indexByUrl : indexByMeta;
    ...
    if (snapshot.playing) {
      player.play(targetIndex);
      window.setTimeout(() => {
        player.seek(snapshot.time || 0);
      }, 120);
    }
  }, [tracks, player]);
