...
**SEO & Search Engines**:
- **Site URL** is set in `config/site.yaml` → `site.url`. This must be the real domain — it's used for sitemap, canonical URLs, RSS, and OG tags.
- **Sitemap**: `@astrojs/sitemap` v3 generates `sitemap-index.xml` (NOT `sitemap.xml`). Auto-generated on every `pnpm build`. Submit `sitemap-index.xml` to Google Search Console.
- **SEO meta tags**: Managed by `astro-seo` in `src/layouts/Layout.astro` (title, description, OG, Twitter cards, canonical URLs). Each post's description comes from its frontmatter `description` field.
- **robots.txt**: Auto-generated by `astro-robots-txt`, configured via `seo.robots` in `config/site.yaml`.
- **Google Search Console**: Verified via HTML meta tag in `Layout.astro` `<head>`.
- **RSS**: Available at `/rss.xml`, linked in `<head>`.
- **hreflang**: Auto-generated for multi-language SEO when i18n is enabled.
