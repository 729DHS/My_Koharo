...
```markdown
---
title: 我的第一篇文章 # 必填：文章标题
link: my-first-post # 可选：文章固定链接 slug；建议小写+短横线，发布后尽量不要改
date: 2024-01-01 12:00:00 # 必填：发布时间（Asia/Shanghai）
...
categories: # 可选：分类（推荐写）
  - [笔记, 前端] # 嵌套分类示例；分类名需在 config/site.yaml 的 categoryMap 中有映射
  # - 随笔 # 单层分类示例（二选一）

sticky: false # 可选：是否置顶；默认 false（不置顶），通常可省略
draft: false # 可选：是否草稿；默认 false（发布），通常可省略

math: false # 可选：是否启用公式渲染（内容含 KaTeX 时再打开）
catalog: true # 可选：是否生成目录
---
...
| `cover`       | ❌   | 封面图片路径                    |
| `description` | ❌   | 文章摘要                        |
| `sticky`      | ❌   | 设为 `true` 置顶文章（默认 `false`） |
| `draft`       | ❌   | 设为 `true` 标记为草稿（默认 `false`） |
